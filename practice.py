...
def total_meal_price(meal_price, percentage):
    meal_price = raw_input("Please enter Meal Price >>>")
    tip = raw_input("Please enter Tip percentage>>>")
    
    if tip == 0 or tip == []:
        total_amount_paid = meal_price + meal_price*(15.00/100) 
    else:
        total_amount_paid = meal_price + meal_price*(float(tip)/100)
...
def sign_and_parity(int1):
    ...
    if int1 < 0:
        new_number_sign = "Negative"
        if int1%2 == 0:
            new_number_parity = "Even"
        else:
            new_number_parity = "Odd"
    else:
        new_number_sign = "Positive"
        if int1%2 == 0:
            new_number_parity = "Even"
        else:
            new_number_parity = "Odd"
    # new_number = new_number_sign, new_number_parity

    sign_parity.append(new_number_sign)
    sign_parity.append(new_number_parity)

    return sign_parity

sign, parity = sign_and_parity(int1)
